...
void init_linear_actuator()
{
    ...
    detect_LS_TimerA = xTimerCreate("Detect limit Switch A state Polling",(DETECT_LS_POLLING_PERIOD), pdTRUE, ( void * ) 3,  detect_LS_A);
    detect_LS_TimerB = xTimerCreate("Detect limit Switch B state Polling",(DETECT_LS_POLLING_PERIOD), pdTRUE, ( void * ) 3,  detect_LS_B);
}
...
int get_Linear_Actuator_A_LS_State(void)
{
     ...
     * 01 :  upper limited
     * 02 :  lower limited
     * 03 :  both limited, but is impossible actually.
     */
